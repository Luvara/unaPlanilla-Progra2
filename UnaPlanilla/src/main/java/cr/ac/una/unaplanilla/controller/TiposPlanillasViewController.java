 ...
 * @author UNA
 */
public class TiposPlanillasViewController extends Controller implements Initializable {

    @FXML
    private AnchorPane root;
    @FXML
    private TabPane tabPaneTipoPlanilla;
    @FXML
    private Tab tabTipoPlanilla;
    @FXML
    ...
    @FXML
    private TextField txtPlanillasMes;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       txtId.setTextFormatter(Formato.getInstance().integerFormat());
        ...
        txtNombre.setTextFormatter(Formato.getInstance().maxLengthFormat(40));
    ...
    @FXML
    private void OnKeyPressedIdPlanilla(KeyEvent event) {
    ...
    private void selectionChangeTabEmp(Event event) {
        if (tbpInclusionEmpleados.isSelected()) {
            if (tipoPlanillaDto.getId()== null) {
                new Mensaje().showModal(Alert.AlertType.ERROR, "Tipo Planilla", getStage(), "Debe cargar el tipo planilla al que desea agregar empleados.");
                tabPaneTipoPlanilla.getSelectionModel().select(tabTipoPlanilla);
            }
        }
    }
    ...
    private void OnActionBtnNuevo(ActionEvent event) {
        if (tbpInclusionEmpleados.isSelected()) {
            nuevoEmpleado();
        } else if (tabTipoPlanilla.isSelected()) {
    ...
    private void OnActionBtnBuscar(ActionEvent event) {
        FlowController.getInstance().goViewInWindow("BuscadorPlanillaView");
    }
    ...
    private void OnActionBtnEliminar(ActionEvent event) {
         try {
            ...
            Logger.getLogger(TiposPlanillasViewController.class.getName()).log(Level.SEVERE, "Error eliminando el tipo planilla.", ex);
    ...
    public String validarRequeridos() {
        ...
        for (Node node : requeridos) {
            if (node instanceof JFXTextField && ((JFXTextField) node).getText().isBlank()) {
                if (validos) {
                ...
                validos = false;
            } else if (node instanceof JFXPasswordField && ((JFXPasswordField) node).getText().isEmpty()) {
                if (validos) {
    ...
    private void cargarEmpleados() {
     ...
     private void cargarEmpleado(Long id) {
        try {
            ...
            Respuesta respuesta = service.getDatosId(id);
            if (respuesta.getEstado()) {
                unbindEmpleado();
     ...
     public void bindBuscar() {
        BuscadorPlanillaViewController buscadorPlanilla = (BuscadorPlanillaViewController) FlowController.getInstance().getController("BuscadorPlanillaView");
      ...
      private class ButtonCell extends TableCell<EmpleadoDto, Boolean> {
        ...
        ButtonCell() {
            ...
            cellButton.getStyleClass().add("mfx-btnimg-ltveliminar");
